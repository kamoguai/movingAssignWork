...
class _CalendarSelectorDialogState extends State<CalendarSelectorDialog>
  ...
  ///選擇日期後event
  void _onDaySelected(DateTime day, List events, List none) {
          ...
          children: <Widget>[
            Card(
              ...
              child: BookingResultDialog(
                custNo: widget.custNoStr,
                originDate: widget.bookingDate,
                changeDate: timePeriodArr[0],
                dataModel: dataModel,
  ...
  Widget build(BuildContext context) {
    ...
    columnList2.add(_buildTableCalendar());
    columnList2.add(
      Container(
      ...
      child: TabBarView(
        ...
        children: <Widget>[
          TimePeriodItem(
            key: ValueKey('class1'),
            classStr: "早",
            modelList: class1List,
            addTransform: _addTransform,
            timePeriodArr: timePeriodArr,
            selectDate: _selectedDay,
            fromFunc: widget.fromFunc,
          ),
          TimePeriodItem(
            key: ValueKey('class2'),
            classStr: "中",
            modelList: class2List,
            addTransform: _addTransform,
            timePeriodArr: timePeriodArr,
            selectDate: _selectedDay,
            fromFunc: widget.fromFunc,
          ...
          Expanded(
            child: Container(
              ...
              child: FlatButton(
                ...
                color: isValid == true
                    ? Color(MyColors.hexFromStr('40b89e'))
                    : Colors.grey,
                child: Text(
                  '確定',
                  style: TextStyle(
                ...
                onPressed: () async {
                  if (isValid) {
                    ...
                    if (widget.wkNoStr != null) {
                      ...
                      var res = await _postModifyBookingDate(
                          timePeriodArr[0], inputText);
                      Navigator.pop(context);
                      if (res.result) {
                        Navigator.pop(context);
                        showDialog(
...
class TimePeriodModel {
  ...
  TimePeriodModel.forMap(data) {
    ...
    timePeriod =
        data["timePeriod"] == null ? "" : data["timePeriod"].toString();
    acceptedAmount =
        data["acceptedAmount"] == null ? "" : data["acceptedAmount"].toString();
